...
/**
 * Seed assessment types that match the UI in assessments.tsx
 */
export async function seedUIAssessments() {
  const uiAssessments = [
    {
    ...
    },
    {
      ...
      coachTypes: ["Weight Loss Coach", "Wellness Coach"],
      isActive: true
    }
  ];

  try {
    console.log("Seeding UI assessment types...");
    
    for (const assessment of uiAssessments) {
      try {
        const existing = await storage.getAssessmentTypeById(assessment.id);
        
        if (existing) {
          console.log(`✓ Assessment type "${assessment.displayName}" already exists`);
        } else {
          await storage.createAssessmentType(assessment);
          console.log(`✓ Created assessment type: ${assessment.displayName}`);
        }
      } catch (error) {
        console.error(`Error creating assessment type "${assessment.displayName}":`, error);
      }
    }
    
    console.log(`\n✅ Successfully seeded ${uiAssessments.length} UI assessment types`);
  } catch (error) {
    console.error("Error seeding UI assessment types:", error);
    throw error;
  }
}
...
if (isMainModule) {
  seedUIAssessments()
    .then(() => {
      console.log("UI assessment types seeding completed");
      process.exit(0);
    })
    .catch((error) => {
      console.error("Failed to seed UI assessment types:", error);
      process.exit(1);
    });
}
